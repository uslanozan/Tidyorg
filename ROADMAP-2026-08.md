...
| :--- | :--- | :--- |
| [`config/organization.example.yml`](terraform/config/organization.example.yml) → `defaults.visibility` | `public` → `private` | **1 line** |
| `config/repositories/pilot-access-test.yml` _(historical; removed)_ | `protected_branches: {main,develop} = null` can be removed | 3 lines _(the repo is already temporary)_ |
